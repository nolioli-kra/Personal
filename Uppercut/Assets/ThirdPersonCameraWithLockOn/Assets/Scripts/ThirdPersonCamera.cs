...
namespace ThirdPersonCameraWithLockOn
{
	...
	public class ThirdPersonCamera : MonoBehaviour
	{
        ...
        // Method to get the position of the current target
        public Vector3 GetCurrentTargetPosition()
        {
            if (lockOnTargets != null && LockOnTargets.Length > 0)
            {
                if (LockOnCurrent >= 0 && LockOnCurrent < lockOnTargets.Length)
                {
                    // Ensure that the current target index is within the bounds of the array
                    GameObject currentTarget = lockOnTargets[LockOnCurrent];
                ...
                {
                    Debug.LogWarning("Invalid current target index!");
                    return Vector3.zero;
                }
            }
            else
